...
exports.sendPairingSubmitBinary = exports.sendPairingModeStartBinary = exports.decodePairingAck = exports.encodePairingSubmit = exports.encodePairingStart = void 0;
const dgram_1 = __importDefault(require("dgram"));
const remp_1 = require("./remp");
...
const decodePairingStatus = (value) => {
    if (value === 0)
        return "ok";
    if (value === 1)
        return "invalid_pin";
...
const encodePairingStart = (corrId) => {
    const corrBytes = (0, remp_1.uuidToBytes)(corrId);
...
exports.encodePairingStart = encodePairingStart;
const encodePairingSubmit = (corrId, pin, pairingNonce, dongleToken) => {
    assertPin(pin);
    const corrBytes = (0, remp_1.uuidToBytes)(corrId);
...
exports.encodePairingSubmit = encodePairingSubmit;
const decodePairingAck = (message) => {
    const { type, payloadOffset } = (0, remp_1.decodeRempHeader)(message);
    if (type !== remp_1.REMP_TYPE_PAIRING) {
    ...
    const corrId = (0, remp_1.bytesToUuid)(payload.subarray(4, 4 + CORR_ID_LEN));
    let pairingNonce;
...
exports.decodePairingAck = decodePairingAck;
const sendUdpRequest = async (target, message, timeoutMs = DEFAULT_TIMEOUT_MS) => {
...
const sendPairingModeStartBinary = async (target, dongleId, corrId) => {
    const payload = (0, exports.encodePairingStart)(corrId);
    const header = (0, remp_1.encodeRempHeader)({ type: remp_1.REMP_TYPE_PAIRING, deviceId: (0, remp_1.deviceIdToBytes)(dongleId) });
    const response = await sendUdpRequest(target, Buffer.concat([header, payload]));
    const decoded = (0, exports.decodePairingAck)(response);
    if (decoded.corrId !== corrId) {
        throw new Error("Pairing ACK corr_id mismatch.");
    }
    return decoded;
};
exports.sendPairingModeStartBinary = sendPairingModeStartBinary;
const sendPairingSubmitBinary = async (target, dongleId, corrId, pin, pairingNonce, dongleToken) => {
    const payload = (0, exports.encodePairingSubmit)(corrId, pin, pairingNonce, dongleToken);
    const header = (0, remp_1.encodeRempHeader)({ type: remp_1.REMP_TYPE_PAIRING, deviceId: (0, remp_1.deviceIdToBytes)(dongleId) });
    const response = await sendUdpRequest(target, Buffer.concat([header, payload]));
    const decoded = (0, exports.decodePairingAck)(response);
    if (decoded.corrId !== corrId) {
...
exports.sendPairingSubmitBinary = sendPairingSubmitBinary;
